...
@author: Integration of Gatekeeper with Highway Environment
============================================
Highway scenario using Gatekeeper algorithm for safe control.
Combines the highway environment with Gatekeeper's nominal/backup trajectory planning.
"""
...
# ========================================================================
# Highway Controller with Gatekeeper
# ========================================================================
class HighwayGatekeeperController:
    def __init__(self, X0, robot_spec, highway_env, dt=0.05, 
        ...
        # Set controllers in Gatekeeper
        self.gatekeeper._set_nominal_controller(self.nominal_controller_wrapper)
        self.gatekeeper._set_backup_controller(self.backup_controller_wrapper)
    ...
    def nominal_controller_wrapper(self, state, goal):
        """
        Wrapper for Gatekeeper to call nominal controller.
        Gatekeeper expects: nominal_controller(state, goal) -> control (nu, 1)
        """
        return self.nominal_input_highway(target_speed=self.robot_spec.get('v_nominal', 2.0))

    def backup_controller_wrapper(self, state):
        """
        Wrapper for Gatekeeper to call backup controller (lane change).
        Gatekeeper expects: backup_controller(state) -> control (nu, 1)
        """
    ...
    def control_step(self):
        """Execute one control loop iteration using Gatekeeper."""
        self._advance_traffic()

        # Get safe control from Gatekeeper (control_ref defined once in set_waypoints)
        u_safe = self.gatekeeper.solve_control_problem(
            self.robot.X, 
            self.control_ref, 
            self.obs
        )
    ...
    def draw_plot(self, pause=0.01):
        ...
        if hasattr(self.gatekeeper, 'committed_x_traj') and self.gatekeeper.committed_x_traj is not None:
            ...
            if hasattr(self.gatekeeper, 'committed_horizon'):
                nominal_len = int(self.gatekeeper.committed_horizon / self.dt)
                ...
                # Draw backup trajectory (orange)
                if nominal_len < len(traj):
                    line, = self.ax.plot(
                        traj[nominal_len:, 0], traj[nominal_len:, 1],
                        color='orange', linestyle='--', linewidth=2.0, alpha=0.8, zorder=2,
...
# ========================================================================
def highway_gatekeeper_main(save_animation=False):
    """Run the highway scenario with Gatekeeper control."""
    ...
    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.set_title("Highway Scenario with Gatekeeper Algorithm")
    ...
    # Create controller with Gatekeeper
    controller = HighwayGatekeeperController(
        X0=x0,
    ...
    # Run simulation loop
    print("Starting highway scenario simulation with Gatekeeper...")
    print(f"Nominal horizon: {controller.gatekeeper.nominal_horizon}s")
    print(f"Backup horizon: {controller.gatekeeper.backup_horizon}s")
    print(f"Event offset: {controller.gatekeeper.event_offset}s")
...
if __name__ == "__main__":
    highway_gatekeeper_main(save_animation=False)
