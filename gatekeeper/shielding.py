...
class Shielding:
    def __init__(self, robot, dt=0.05, nominal_horizon=2.0, backup_horizon=2.0, event_offset=1.0):
        """
        Initialize the Shielding controller.
        ...
        # Visualization support for backup trajectories
        self.visualize_backup = False
        self.backup_trajs = []
        self.save_every_N = 1  # Save more frequently for better visualization
        self.curr_step = 0
    ...
    def solve_control_problem(self, robot_state, control_ref, nearest_obs):
        ...
        if self.current_time_idx > self.next_event_time/self.dt:
            ...
            # SHIELDING ALGORITHM:
            # Roll out nominal step-by-step, checking if backup after each step is safe
            # Continue until backup becomes unsafe, then commit i-1
            
            last_safe_i = 0
            
            # Try progressively longer nominal horizons
            for i in range(int(self.nominal_horizon // self.horizon_discount) + 1):
                ...
                # Check if ENTIRE candidate trajectory (nominal + backup) is safe
                if self._is_candidate_valid(candidate_x_traj, nearest_obs):
                    # This nominal+backup is safe - remember i and continue
                    last_safe_i = i
                else:
                    # Backup became unsafe - stop and use i-1
                    break
            ...
            if self.visualize_backup and self.curr_step % self.save_every_N == 0:
                nominal_len = int(discounted_nominal_horizon / self.dt)
                if nominal_len < len(self.candidate_x_traj):
                    backup_portion = self.candidate_x_traj[nominal_len:]
