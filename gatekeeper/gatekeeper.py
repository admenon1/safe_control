...
class Gatekeeper:
    ...
    def _is_candidate_valid(self, candidate_x_traj, unsafe_region, obs_safety_margin=0.1):
        """
        ...
        if self.current_time_idx > self.next_event_time/self.dt:
            ...
            for i in range(int(self.nominal_horizon//self.horizon_discount)):
                ...
                if self._is_candidate_valid(candidate_x_traj, nearest_obs):
                    ...
                    # Store backup trajectory for visualization (after finding valid candidate)
                    if self.visualize_backup and self.curr_step % self.save_every_N == 0:
                        # Extract backup portion from candidate trajectory
